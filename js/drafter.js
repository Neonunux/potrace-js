...
extend(Drafter, {
	...
	considerLeftTurn: function (index, direction) {
		...
		if (direction === 'N')
			check = self.coord(check - self.width);
		if (direction === 'E')
			check = self.coord(check + 1);
		if (direction === 'S')
			check = self.coord(check + self.width);
		if (direction === 'W')
			check = self.coord(check - 1);
	...
	considerRightTurn: function (index, direction) {
		...
		var check = self.index(index);
		if (direction === 'N')
			check = self.coord(check - self.width + 1);
		if (direction === 'E')
			check = self.coord(check + self.width + 1);
		if (direction === 'S')
			check = self.coord(check + self.width - 1);
	...
	takeRightTurn: function (index, direction) {
		...
		var check = self.index(index);
		if (direction === 'N')
			return self.coord(check - self.width + 1);
		if (direction === 'E')
			return self.coord(check + self.width + 1);
		if (direction === 'S')
			return self.coord(check + self.width - 1);
	...
	goStraight: function (index, direction) {
		...
		var check = self.index(index);
		if (direction === 'N')
			return self.coord(check - self.width);
		if (direction === 'E')
			return self.coord(check + 1);
		if (direction === 'S')
			return self.coord(check + self.width);
		if (direction === 'W')
			return self.coord(check - 1);
